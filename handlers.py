...
def handle_event(vk, event, upload):
    ...
    # --- Глобальный перехватчик кнопки Приложения (из-за ограничений VK API на внешние ссылки) ---
    if keyboards.BTN_LABEL in text:
        send_message(
            vk, user_id,
            "🔗 Открывайте наше приложение по ссылке:\n"
            f"👉 {keyboards.LINK}\n\n"
            "Смотрите работы и цены. Если будут вопросы — жмите нужную кнопку ниже 👇"
        )
        return
    ...
    if state == storage.STATE_WELCOME:
        if "Выбрать из меню" in text or "Да, знаю что хочу" in text or "Оформить заявку" in text:
            ...
            welcome_text = (
                ...
                "👇 Нажмите кнопку «Открыть приложение», чтобы получить ссылку на каталог работ и цены:"
            )
